...
int main(int argc, char const *argv[]) {
	...
	*************************************************************************************************/
	...
	/************************************************************************************************
		...
		- Below is Multi-Level Feedback Queue (RR > FCFS)
	************************************************************************************************/

	// NOTE: queue array tracks which scheduling algorithm to use
	// RR 	= 2
	// FCFS = 1
	// Done = 0
	// every task starts at top queue with queue[] = 2
	// FIFO property is guaranteed given the experiment conditions 
	// i.e. the order can't be rearranged without preemption, priority within queues, or promotion
	// - tasks will always execute in 1,2,3,4... order
	// - this would remain the case even with more layers
	// keeping track of queue counts would be necessary if more tasks could arrive later
	...
	while (running[0] > 0 || running[1] > 0 || running[2] > 0 || running[3] > 0) {
		for (int i = 0; i < 4; i++) {
            ...
            if (running[i] > 0) {
                ...
                if (queue[i] == 2) {
					// RR - wait for time-quantum
					usleep(QUANTUM);
					waitpid(pid[i], &running[i], WNOHANG);
				}
				else if (queue[i] == 1) {
					// FCFS - wait until task completion
					waitpid(pid[i], &running[i], 0);
				}
